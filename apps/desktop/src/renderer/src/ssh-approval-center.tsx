    ...
    <div className="ssh-approval-backdrop" ref={backdropRef}>
      ...
      >
        <header className="ssh-approval-dialog-header">
          <div>
            <span className="ssh-approval-kicker">{uiText('REMOTE SERVER APPROVAL')}</span>
            <h2 id={titleId}>{uiText('SSH approval required')}</h2>
          </div>
        ...
        <div className="ssh-approval-dialog-body">
          <article className="ssh-approval-card">
            <div>
              <span>{uiText('SERVER')}</span>
              <strong>{request.connectionLabel}</strong>
              <small>
                {uiText('SSH target ·')} {request.targetDisplay ?? request.hostAlias}
              </small>
              {request.privilegeClass === 'root' && remoteWorkspace && (
                <strong className="ssh-root-warning">
                  {uiText('HIGH RISK · project code can run as ROOT and affect the entire server')}
                </strong>
              )}
              {request.privilegeClass === 'root' && !remoteWorkspace && (
                <strong className="ssh-root-warning">
                  {uiText('ROOT · restricted diagnostics')}
                </strong>
              )}
              {request.privilegeClass === 'unknown' && (
                <strong className="ssh-root-warning">
                  {uiText('HIGH RISK · account/target resolved from SSH configuration')}
                </strong>
              )}
              <small>
              ...
              {remoteWorkspace && (
                <>
                  <small>
                    {uiText('Mode · remote workspace /')}{' '}
                    {request.workspaceOperation ?? uiText('unknown')}
                    {workspaceFileAction ? ` / ${workspaceFileAction}` : ''}
                  </small>
                  <small>
                    {uiText('Configured root ·')} {request.workspaceRoot}
                  </small>
                  <small>
                    {uiText('Exact working directory ·')} {request.workspaceWorkingDirectory}
                  </small>
                  {workspaceFileAction && (
                    <small>
                      {uiText('File action ·')} {workspaceFileAction.toUpperCase()}
                    </small>
                  )}
                  {request.workspaceFilePath && (
                    <small>
                      {uiText('Relative file path ·')} {request.workspaceFilePath}
                    </small>
                  )}
                  {request.workspaceFileExpectedSha256 && (
                    <small>
                      {uiText('Expected existing SHA-256 ·')} {request.workspaceFileExpectedSha256}
                    </small>
                  )}
                  {workspaceFileAction === 'create' && !request.workspaceFileExpectedSha256 && (
                    <small>{uiText('Expected existing file · none (create only)')}</small>
                  )}
                  {request.workspaceFileContentSha256 && (
                    <small>
                      {uiText('Approved content SHA-256 ·')} {request.workspaceFileContentSha256}
                    </small>
                  )}
                  <small>
                    {uiText('Connection v')}
                    {request.connectionVersion} {uiText('· Grant v')}
                    {request.workspaceGrantVersion}
                  </small>
                  <small>
                    {uiText('Request SHA-256 ·')} {request.commandSha256}
                  </small>
                </>
              )}
            </div>
            <pre aria-label={uiText('Requested SSH operation')}>{request.commandPreview}</pre>
            {request.workspaceFileContent !== undefined && (
              <div className="ssh-approved-file-content">
                <strong>{uiText('Exact approved file content')}</strong>
                <pre aria-label={uiText('Exact approved SSH file content')}>
                  ...
                  : inspectsWorkspaceFile
                    ...
                    : runsForegroundExperiment
                      ? uiText(
                          ...
                          { SSH_COMMAND_MAX_TIMEOUT_SECONDS: SSH_COMMAND_MAX_TIMEOUT_SECONDS },
                        )
                      : executesWorkspaceCode
                        ? uiText(
                            ' This test/build can execute untrusted project code and change server state.',
                          )
                        : uiText(' This inspection may still expose private repository data.')}{' '}
                {executesWorkspaceCode &&
                  uiText(
