...
[0.58.163 설치](releases/0.58.163.md): 어느 화면이 어느 패널을 보여주는지 아무 테스트도 보고 있지 않으면, 그만큼 눈에 띄는 동작을 바꿔도 전부 통과한다. 통과가 곧 안전은 아니다. 그리고 설명을 지우는 것과 화면에서 내리는 것은 다르다. 툴팁으로 옮기면 정보는 남는다

[0.58.162 설치](releases/0.58.162.md): 접힌 disclosure의 본문은 화면에 안 보여도 만드는 비용은 다 낸다. 카드 한 장의 82%가 거기 있었다. 그리고 스크롤 복원값에 방금 스크롤한 결과를 다시 써 넣으면 안 된다. 레이아웃이 덜 자란 상태에서는 그 값이 잘린 값이고, 한 번 덮어쓰면 원래 위치는 복구할 수 없다
